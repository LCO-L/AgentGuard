...
from api.deps import ag_client, ai_config, check_size, verify_api_key
...
async def scan(file: UploadFile = File(...),
    ...
    try:
        ...
        verdict = await run_in_threadpool(
            scan_service.scan_bytes, data, file.filename or "upload.bin", cfg, client)
...
async def scan_batch(files: list[UploadFile] = File(...),
    ...
    return {"results": await run_in_threadpool(scan_service.scan_batch, payload, cfg, client)}
